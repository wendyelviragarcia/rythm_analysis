    ...
    tryCatch(
      {
        ...
        for (loopIndex in 1:nFiles){
          ...
          #textgrid= readTextGridRobust("/Users/weg/Desktop/untitled.TextGrid", "UTF-16")[[2]]
          ...
          textgrid <- tryCatch(
            expr = {
              ...
              textgrid <- readTextGridRobust(filetoRead, "UTF-16BE")[[{input$tier}+1]]
            },
            warning = function(w){
              # (Optional)
              # Do this if an error is caught...
              message("Error de lectura, buscando codificación de manera automática...")
              # Choose a return value in case of error
              encoding = guess_encoding(filetoRead) %>% filter(confidence == 1) %>% pluck("encoding")
          ...
          if (input$annotation == 1){
            ...
            textgrid$Outcomes <- gsub( "[aeoiujwəεɑɔ]", "V", textgrid$Outcomes)
             ...
             textgrid$Outcomes <- gsub( "[aeiou]", "V", textgrid$Outcomes)
          ...
          if (identical(speechRate, numeric(0))){
            speechRate = 0
          }
